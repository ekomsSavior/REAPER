...
var (
	...
	scanIssues     = flag.Bool("scan-issues", true, "Scan issue titles and bodies")
	scanCommits    = flag.Bool("scan-commits", true, "Scan commit messages")
	...
	maxFileBytes   = flag.Int64("max-file-bytes", 1<<20, "Maximum repository file size to scan")
	apiRPS         = flag.Float64("api-rps", 1, "Maximum average GitHub API requests per second")
	apiBurst       = flag.Int("api-burst", 1, "Maximum burst of GitHub API requests")
	rateBuffer     = flag.Duration("rate-limit-buffer", 5*time.Second, "Extra delay after a GitHub rate-limit reset")

	// Single/multiple repo scanning
	repoURLs       stringListFlag
	repoListFile   = flag.String("repo-list", "", "File containing repository URLs (one per line)")
	searchKeywords stringListFlag
	wordlistFile   = flag.String("wordlist", "", "File containing repository search keywords (one per line)")
	promptKeywords = flag.Bool("prompt-keywords", false, "Interactively prompt for search keywords or a wordlist")
	ruleFiles      stringListFlag
	rulesOnly      = flag.Bool("rules-only", false, "Use only rules loaded with -rules")
)

func init() {
	flag.Var(&searchKeywords, "keyword", "Repository search keyword (repeatable or comma-separated)")
	flag.Var(&ruleFiles, "rules", "JSON detector rule pack (repeatable)")
	flag.Var(&repoURLs, "repo", "Repository URL to scan (repeatable)")
}
...
type Reaper struct {
	...
	mu            sync.Mutex
	rateMu        sync.Mutex
	rateResume    time.Time
	closeOnce     sync.Once
	scannedRepos  map[string]bool
	inFlightRepos map[string]bool
	findingCache  map[string]bool
...
func main() {
	...
	if err := configureSearchKeywords(); err != nil {
		log.Fatalf("Failed to configure search keywords: %v", err)
	}
	...
	for _, path := range ruleFiles {
		...
		if err != nil {
			log.Fatalf("Failed to load detector rules: %v", err)
		}
		patterns = append(patterns, loaded...)
	}
	if len(patterns) == 0 {
		log.Fatal("No detector rules configured")
	}

	reaper := NewReaper(token, patterns)
    ...
    ╚═╝  ╚═╝╚══════╝╚═╝  ╚═╝╚═╝     ╚══════╝╚═╝  ╚═╝
    ============================================================
    REAPER Core v2 - GitHub Secret & Advisory Scanner
    ============================================================
    `)
	...
	if len(targetRepos) > 0 {
		...
		for _, repoURL := range targetRepos {
			...
			if err := reaper.scanRepository(repo); err != nil {
				log.Printf("Failed to scan %s: %v", repo.GetFullName(), err)
				continue
			}
	...
	if len(searchKeywords) > 0 {
		fmt.Printf("[+] Search keywords (%d): %s\n", len(searchKeywords), strings.Join(searchKeywords, ", "))
	} else {
		fmt.Println("[+] Search keywords: none (broad repository discovery)")
	}
...
func configureSearchKeywords() error {
	if *wordlistFile != "" {
		words, err := readKeywordFile(*wordlistFile)
		if err != nil {
			return err
		}
		searchKeywords = append(searchKeywords, words...)
	}

	if *promptKeywords {
		fmt.Print("Search keywords (comma-separated, @path for a wordlist, blank for broad search): ")
		...
		if scanner.Scan() {
			...
			if strings.HasPrefix(input, "@") {
				...
				if path == "" {
					return fmt.Errorf("wordlist path after @ cannot be empty")
				}
				words, err := readKeywordFile(path)
				if err != nil {
					return err
				}
				searchKeywords = append(searchKeywords, words...)
			} else if input != "" {
				if err := searchKeywords.Set(input); err != nil {
					return err
				}
			}
		}
		if err := scanner.Err(); err != nil {
			return fmt.Errorf("read keyword prompt: %w", err)
		}
	}
...
func uniqueKeywords(words []string) []string {
...
func getTargetRepos() []string {
	repos := append([]string(nil), repoURLs...)
...
func (r *Reaper) getRepoFromURL(rawURL string) (*github.Repository, error) {
	...
	for {
		if err := r.waitForAPIPermit(); err != nil {
			return nil, err
		}
		...
		if !r.pauseForRateLimit(err) {
			return nil, err
		}
	}
}

func NewReaper(token string, patterns []*SecretPattern) *Reaper {
	...
	reaper := &Reaper{
		client:        client,
		detectors:     detectorsFromPatterns(patterns),
		...
		findingCache:  make(map[string]bool),
	...
	go func() {
		...
		reaper.processResults()
	}()
	go func() {
		defer reaper.wg.Done()
		reaper.processAdvisories()
	}()

	return reaper
}

func (r *Reaper) waitForAPIPermit() error {
	for {
		r.rateMu.Lock()
		resume := r.rateResume
		r.rateMu.Unlock()

		if wait := time.Until(resume); wait > 0 {
...
func (r *Reaper) pauseForRateLimit(err error) bool {
	resume, reason, limited := rateLimitResume(err, time.Now(), *rateBuffer)
	if !limited {
		return false
	}

	r.rateMu.Lock()
	extended := resume.After(r.rateResume)
	if extended {
		r.rateResume = resume
	}
	r.rateMu.Unlock()
	...
	if extended {
		...
		r.stats.mu.Unlock()
		log.Printf("GitHub %s; pausing all API workers until %s", reason, resume.Format(time.RFC3339))
	}
	return true
}

func rateLimitResume(err error, now time.Time, buffer time.Duration) (time.Time, string, bool) {
	var primary *github.RateLimitError
	if errors.As(err, &primary) {
		resume := primary.Rate.Reset.Time
		if resume.Before(now) {
			resume = now
		}
		return resume.Add(buffer), "primary rate limit reached", true
	}

	var secondary *github.AbuseRateLimitError
	if errors.As(err, &secondary) {
		retryAfter := secondary.GetRetryAfter()
		if retryAfter <= 0 {
			retryAfter = time.Minute
		}
		return now.Add(retryAfter).Add(buffer), "secondary rate limit reached", true
	}
...
func (r *Reaper) scanCycle() error {
	for _, query := range r.buildSearchQueries() {
		fmt.Printf("[+] Search query: %s\n", query)
		if err := r.scanQuery(query); err != nil {
			return err
		}
	}
...
func (r *Reaper) scanQuery(query string) error {
	...
	for {
		...
		for _, repo := range result.Repositories {
			...
			r.mu.Lock()
			if r.scannedRepos[repo.GetFullName()] || r.inFlightRepos[repo.GetFullName()] {
				r.mu.Unlock()
				continue
			}
			r.inFlightRepos[repo.GetFullName()] = true
			r.mu.Unlock()
...
func (r *Reaper) repoWorker(repoChan <-chan *github.Repository, wg *sync.WaitGroup) {
	...
	for repo := range repoChan {
		...
		err := r.scanRepository(repo)
		r.mu.Lock()
		delete(r.inFlightRepos, repo.GetFullName())
		if err == nil {
...
func (r *Reaper) scanRepository(repo *github.Repository) error {
	...
	if err := r.scanBranch(repoName, defaultBranch); err != nil {
		return err
	}
...
func (r *Reaper) scanBranch(repoName, branch string) error {
	if branch == "" {
	...
	for {
		...
		tree, _, err = r.client.Git.GetTree(
			r.ctx, getOwner(repoName), getRepoName(repoName), branch, true)
		if err == nil {
			break
		}
		if !r.pauseForRateLimit(err) {
			return fmt.Errorf("fetch tree for %s: %w", branch, err)
		}
	}
	if tree.GetTruncated() {
		return fmt.Errorf("repository tree is truncated; refusing to record an incomplete scan")
	}
	...
	for _, entry := range tree.Entries {
		...
		if entry == nil || entry.GetType() != "blob" || shouldSkipPath(entry.GetPath()) {
			continue
		}
		if entry.GetSize() > 0 && int64(entry.GetSize()) > *maxFileBytes {
			continue
		}
		var content []byte
		for {
			...
			var err error
			content, _, err = r.client.Git.GetBlobRaw(
				r.ctx, getOwner(repoName), getRepoName(repoName), entry.GetSHA())
			if err == nil {
				break
			}
			if !r.pauseForRateLimit(err) {
				return fmt.Errorf("fetch %s: %w", entry.GetPath(), err)
			}
		}
		...
		url := fmt.Sprintf("https://github.com/%s/blob/%s/%s", repoName, branch, entry.GetPath())
		r.scanContent(repoName, entry.GetPath(), branch, string(content), url)
	}
	return nil
}
...
func (r *Reaper) scanText(repoName, location, text, url string) {
	r.scanContent(repoName, location, "", text, url)
}

func (r *Reaper) scanContent(repoName, location, branch, content, url string) {
	for _, detector := range r.detectors {
		for _, detection := range detector.Detect(content) {
			if detection.Type == "Email Address" {
				if *hideObfuscated && isObfuscatedEmail(detection.Secret) {
					continue
				}
				if isGitHubNoReply(detection.Secret) || r.isDuplicateEmail(repoName, detection.Secret) {
					continue
				}
			}

			hash := sha256.Sum256([]byte(
				repoName + "\x00" + location + "\x00" + branch + "\x00" + detection.Type + "\x00" + detection.Secret))
			id := hex.EncodeToString(hash[:])[:16]
			if r.isDuplicateFinding(id) {
				continue
			}
			...
			r.results <- &Finding{
				...
				SecretType:  detection.Type,
				SecretValue: maskSecret(detection.Secret),
...
func (r *Reaper) isDuplicateFinding(id string) bool {
	r.mu.Lock()
	defer r.mu.Unlock()
	if r.findingCache[id] {
		return true
	}
	r.findingCache[id] = true
	return false
}
...
func (r *Reaper) buildSearchQueries() []string {
	keywords := []string{"a"}
	if len(searchKeywords) > 0 {
	...
	for _, keyword := range keywords {
		query := quoteSearchKeyword(keyword) + " is:public"
		if *sinceDays > 0 {
...
func quoteSearchKeyword(keyword string) string {
